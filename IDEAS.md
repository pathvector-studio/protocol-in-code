...
進行状況: DNS / TCP / TLS / HTTP-QUIC = **完了（2026-07-11 → COURSE_MAP.md へ移行）**。
TODO.md 記載の4トラックは全て消化済み。次は下の「新トラック候補」から確定して投入
（推し順: Packet Parser → RPKI）。
...
### Track: Packet Parser — 5 modules（全トラックの土台、最優先候補）

1. **Bytes have a shape** — ヘッダとは offset の約束である / `parser/ethernet.py`
2. **Peel one layer, find the next** — EtherType/Protocol 番号による分岐 / `parser/dispatch.py`
3. **Bits don't align to bytes** — フラグとビット演算 / `parser/ip.py`
4. **The checksum is arithmetic, not magic** — 1の補数和を書く / `parser/checksum.py`
5. **Build the toy pcap reader** — pcap を自力で読む総集編 / `parser/pcap_loop.py`

### Track: RPKI — 5 modules（BGP トラックの validation.py に接続、既存資産が活きる）

1. **A ROA is a permission slip** — ROA の入力構造 / `rpki/roa.py`
2. **Covering is prefix math** — prefix 包含と maxLength / `rpki/covering.py`
3. **Three verdicts, not two** — valid/invalid/not_found の tri-state / `rpki/validate.py`
4. **Policy decides what a verdict means** — 判定の使い方は別の関数 / `rpki/policy.py`
5. **Build the toy validator loop** — BGP トラックと合流する総集編 / `rpki/validator_loop.py`
